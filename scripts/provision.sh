...
argocd() {
...

  {
    ...
    echo "    create: true"
    echo
    echo "## Redis-HA subchart replaces custom redis deployment when \`redis-ha.enabled=true\`"
    echo "# Ref: https://github.com/DandyDeveloper/charts/blob/master/charts/redis-ha/values.yaml"
    echo "redis-ha:"
    echo "  # -- Enables the Redis HA subchart and disables the custom Redis single node deployment"
    echo "  enabled: true"
    echo "  ## Prometheus redis-exporter sidecar"
    echo "  exporter:"
    echo "    # -- Enable Prometheus redis-exporter sidecar"
